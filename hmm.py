...
def viterbi(w, a, b, c):
    ...
    delta = zeros([L, I, J])
    index = zeros([L, I, J], dtype=int)
    best_delta = zeros([L])
    best_state = zeros([L, I], dtype=int)
    for l in range(0, L):
        for j in range(0, J):
            delta[l, 0, j] = b[j]*c[j, w[l][0]]
        for i in range(1, len(w[l])):
            for j in range(0, J):
                previous = -1
                for j_prime in range(0, J):
                    this = delta[l, i-1, j_prime]*a[j_prime, j]
                    if this>=delta[l, i, j]:
                        delta[l, i, j] = this
                        previous = j_prime
                index[l, i, j] = previous
                delta[l, i, j] *= c[j, w[l][i]]
        best = 0
        best_index = -1
        for j in range(0, J):
            this =  delta[l, len(w[l])-1, j]
            if this>=best:
                best = this
                best_index = j
        best_delta[l] = best
        for i in range(len(w[l])-1, -1, -1):
            best_state[l, i] = best_index
            best_index = index[l, i, best_index]
