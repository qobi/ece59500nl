...
def strip(args):
    ...
    if category in lexicon[words[0]]:
        possibilities.append(((category, words[0]), words[1:]))
    for A, B, C in grammar:
        if A==category:
            for possibility1, words_prime1 in strip((words, B)):
                for possibility2, words_prime2 in strip((words_prime1, C)):
                    possibilities.append(((category, possibility1, possibility2),
                                          words_prime2))
...
def recursive_descent(words, category):
    possibilities = []
    for possibility, words in strip((words, category)):
        if len(words)==0:
