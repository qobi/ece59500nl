...
def shift(stack, words):
    if len(words)==0:
        ...
        for category in lexicon[words[0]]:
            possibilities += repeat((((category, words[0]),)+stack, words[1:]))
        return possibilities

def reduce(stack, words):
    possibilities = []
    for A, B, C in grammar:
        if len(stack)>=2 and stack[0][0]==C and stack[1][0]==B:
            possibilities += repeat((((A, stack[1], stack[0]),)+stack[2:],
                                     words))
    return possibilities
